...
in stdenv.mkDerivation rec {
  ...
  cmakeFlags = if stdenv.system == "x86_64-linux" then [] else ["-DXGL_ENABLE_LTO=OFF"];

  postPatch = if stdenv.system == "x86_64-linux" then "" else ''
    sed -i 's#pal_setup_gcc_ipo()##' drivers/pal/cmake/PalCompilerOptions.cmake
  '';
